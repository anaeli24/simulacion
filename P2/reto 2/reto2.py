...
#    return (alfa+1)*actual[fila,columna]-actual[fila, columna]**3 + (actual[1:-1,2:] + actual[1:-1,:-2] +actual[2:,1:-1] + actual[:-2,1:-1] - 4.*actual[1:-1,1:-1])               
    return (alfa+1)*actual[fila,columna]-actual[fila, columna]**3 + actual[1:-1,1:-1,2:] + actual[1:-1,1:-1,:-2] +actual[1:-1,2:,1:-1] + actual[1:-1,:-2,1:-1] + actual[2:,1:-1,1:-1] + actual[:-2,1:-1,1:-1] -6.*actual[1:-1,1:-1,1:-1]
print(actual)
...
#hasta aqui termina la parte grafica
    ...
    for iteracion in range(50): #num de repeticiones
        ...
        vivos = [paso(x) for x in range(num)]   #regla de la vida
#        vivos = sum(valores)
